...
def image_options(keys=None):
    """Options de liste deroulante pour les images science.
    ...
    n'a pas ete fournie. Avant, elle etait selectionnable, le callback levait
    une exception et Dash conservait le graphique precedent — on croyait
    regarder le spectre du shot choisi alors que c'etait celui d'avant.
    """
    from core.session import SESSION
    bad = SESSION.unusable_images()
    ks = sorted(SESSION.image_dict) if keys is None else list(keys)
    out = []
    for k in ks:
        if k in bad:
            out.append({"label": f"⚠ {k} — unusable file", "value": k,
                        "disabled": True})
        else:
            out.append({"label": k, "value": k})
    return out


def first_usable_image():
